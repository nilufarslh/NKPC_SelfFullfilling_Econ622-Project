...
| 3 | Run the test suite       | `julia --project=. -t auto test/runtests.jl`                    | ~2 min  |
| 4 | Regenerate figures       | `julia --project=. -t auto scripts/fig_report.jl`               | ~30 s   |

Seeds are fixed. Step 2 writes `results/<case>/params.csv`, `results/<case>/moments.csv`, and a timestamped log `results/estimation_<YYYYMMDD_HHMMSS>.log`. The numbers in the US-data table above are reproduced exactly by step 2; the figure in "What the mechanism predicts" is regenerated by step 4.
