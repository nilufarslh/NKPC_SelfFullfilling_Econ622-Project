...
Best estimate from one call to [`estimate_smm`](@ref): the parameter vector,
objective value, convergence flag, number of starts actually attempted, and
a `stage` counter (`1` for the single-stage pipeline used in the headline
results).
"""
struct OptimResult
...
"""
    estimate_smm(cfg, m_data, W, T_obs; inn_d, inn_v, inn_my, inn_mpi,
                 n_starts=cfg.n_starts, verbose=true, warm_start=nothing,
                 logfile=nothing) -> OptimResult

Run multi-start Fminbox(L-BFGS) with ForwardDiff gradients on the SMM
objective. Starts are generated by multiplying `theta0_vec(cfg)` by a fixed
sequence of scalars and clamping into the box; this avoids the degenerate
`κ ≈ 0` basin that low-discrepancy starts on the full box kept finding.
Each start has a two-minute time cap. Returns the start with the lowest
objective.
"""
